...
***			
***
***	\brief description:    Fichier ANSI pour la gestion du fichier LOG
***	
***	
...
*/
/*!                                                                        
**       \brief initialise le fichier FICHIER_LOG                             
**			
**
********************************************************************************/
int init_flog(char *strFile)
{
	int ind;
	
  	if (_TAB_LOG[0].ptr_fichier==NULL)
			ind=0;
		else
			ind=_TAB_LOG[0].nb_element;
		
   _TAB_LOG[ind].ptr_fichier=fopen(strFile,"w");
	 _TAB_LOG[ind].indice=ind;
	 _TAB_LOG[0].nb_element=ind+1; 
...
**        aff_flog                                                
*/                                                                        
/*! \brief  affiche une chaine de caractere a l'ecran et l'inscrit dans le fichier
**	FICHIER_LOG   
**	\param s : cf. printf()
**  \retval 1                          
*******************************************************************************/
int aff_flog(int indice,char *s, ...)
{
	va_list p;
	...
	if(!_TAB_LOG[indice].ptr_fichier)
	{
    #ifdef DEBUG
    printf("WARNING: tried to write in FICHIER_LOG==NULL\n");
    #endif /*DEBUG*/
  }
	else
		vfprintf(_TAB_LOG[indice].ptr_fichier,s,p);
...
*/
/*!                                                                        
**       libere le fichier FICHIER_LOG                             
**	\retval 1
**   \remark la variable global FICHIER_LOG est modifiee
*******************************************************************************/
int end_flog(int indice)
{
	if (_TAB_LOG[indice].ptr_fichier)
		fclose(_TAB_LOG[indice].ptr_fichier);
    _TAB_LOG[indice].ptr_fichier = NULL;
	_TAB_LOG[0].nb_element=_TAB_LOG[0].nb_element-1;	
	return(1);
}
...
**
********************************************************************************/
int init_log(char *strFile)
{
  if (!strFile)
  {
    FICHIER_LOG=NULL;
//    FICHIER_LOG=fopen("result.log","w");
  }
  else
  {
    FICHIER_LOG=fopen(strFile,"w");    
  }
	return(1);
}
...
#ifdef __LOG_DEBUG__
	fprintf(stderr, "FICHIER_LOG: 0x%x\n", FICHIER_LOG);
#endif
...
**
**	Beta, Gamma, Normal, Chi-square, F, Student,...
*******************************************************************************/
...
S120:
	...
	S
	*/
    if(!(*s < 0.0e0 || (*which != 3 && *s > *xn))) goto S160; //verifier si le parenthesage est juste !!
...
S260:
        fx = ccum-*q;
S270:
        if(!((qporq && cum > 1.5e0) || (!qporq && ccum > 1.5e0))) goto S280;  //verifier si le parenthesage est juste !!
        *status = 10;
        return;
S280:
        dinvr(status,shape,&fx,&qleft,&qhi);
...
S90:
    ...
    *status = 0;
    ...
    return;
S100:
    qup = (qincr && yy < 0.0e0) || (!qincr && yy > 0.0e0);  //verifier si le parenthesage est juste !!
	/*
	++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
S240:
    *xhi = c;
    qrzero = (fc >= 0.0e0 && fb <= 0.0e0) || (fc < 0.0e0 && fb >= 0.0e0);//verifier si le parenthesage est juste !!
    if(!qrzero) goto S250;
    *status = 0;
    goto S260;
S250:
...
/*********  End of mathematical special functions part  ****************/

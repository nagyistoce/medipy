...
/* initialisation de la liste des voxels de la fronti�re */
for (i=0;i<wdth;i++)
for (j=0;j<hght;j++)
for (k=0;k<dpth;k++)
	if (im_cl->mri[i][j][k]==-1)
...
while (size>0)
	...
	if (im_cl->mri[i][j][k]==-10)
		...
		size--;
		Ntot--;
		/* mise � jour de la moyenne et du nombre */
		...
		printf("nombre de voxel dans la liste %d   Nb voxel restant � traiter %d  \n",size, Ntot);
	}

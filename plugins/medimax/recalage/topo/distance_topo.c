...
**
**	Calcule de la vraisemblance relative a une boite entre une image labelis�e et une image r�elle
*******************************************************************************/
...
for (i=0;i<HISTOJOINT.nbI;i++)
for (j=0;j<HISTOJOINT.nbJ;j++)
if (HISTOJOINT.aux_hij[i][j]>0.00000001) /* pour �viter des bug num�riques */
...
// On balaye chacun des pixels de la boite de l'image compl�mentaire
for (i=0;i<topDx;i++)
	for (j=0;j<topDy;j++)
		for (k=0;k<topDz;k++)
...
for (i=0;i<topDx;i++)
    ...
    i3=i2+1;j3=j2+1;k3=k2+1;
		...
		/* calcul de la valeur interpol�e dans imreca */
    if (i2>=0 && i3<width && j2>=0 && j3<height && k2>=0 && k3<depth)
      {
    ...
    i3=i2+1;j3=j2+1;k3=k2+1;
		...
		/* calcul de la valeur interpol�e dans imref */
    if (i2>=0 && i3<width && j2>=0 && j3<height && k2>=0 && k3<depth)
      {
...
**
**	Calcule l'energie locale quadratique relative a une boite pour une s�rie DTI
*******************************************************************************/
...
for (i=0;i<topDx;i++)
	for (j=0;j<topDy;j++)
		...
		/* calcul de la valeur interpol�e dans ref et ref2 */
    	dxref=(double)i-lambda_ref*data[i][j][k].x+bx0;
    	dyref=(double)j-lambda_ref*data[i][j][k].y+by0;
    	dzref=(double)k-lambda_ref*data[i][j][k].z+bz0;
     ...
     */
     ...
     for(wx=-hwn;wx<hwn+1;wx++)
         ...
         for(wz=-hwn;wz<hwn+1;wz++)
	   {
		 ...
		 //calcul du champ de d�placement en dehors de la boucle

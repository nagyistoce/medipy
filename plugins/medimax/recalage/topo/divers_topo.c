	...
	for (k=0;k<dpth;k++)
	   {
		...
		//test de la nouvelle fonction d'�valuation du jacobien 
...
** 	inverse_transf_3d                                   
**                                                                   
**	Cette fonction permet de calculer l'inverse d'un champ avec la m�thode   
...
if (p==NULL)
	{
	printf("Probl�me d'allocation dans inv_bspline_3d \n");
	exit(-1);
	}
...
/* verification a posteriori de la pr�cision */
for (i=0;i<wdth;i++)
   for (j=0;j<hght;j++)
    for (k=0;k<dpth;k++)
		{
...
/* estimation de la pr�cision de la transfo inverse */
...
printf("Pourcentage de voxel avec une pr�cision inferieur � %f :  %f pourcent \n",prec,100.0*N/Ntot);
...
**                                                                   
**	Converti une transfo field3d en une transfo Bspline de degr� 1  
** transfo est allou� dans la fonction
*******************************************************************************/
...
**                                                                   
**	Modifie les parametres d'une transfo Bspline pour qu'elle pr�serve la topologie
*******************************************************************************/
...
*********
*********		Evaluation en un point � coordonn�es r�elles d'un champ de d�formation
*********    Bspline de degr� 1
********************************************************************************/
...
/* recopie des param�tres relatif au bloc consid�r� */
...
/* Forme factorisee pour gagner peut-�tre du temps CPU */
/*u->x=
 dzm*(
...
*********
*********		Evaluation en un point � coordonn�es r�elles de la matrice jacobienne 
*********    d'un champ de d�formation de Bspline de degr� 1
********************************************************************************/
...
*/
 ...
 //---- On d�coupe la boite pour que chaque sous-boite soi incluse sur un Slpqr
	...
	if (acknowledge_invbspline(param, nb_param, chres->width, chres->height, chres->depth,i,j,k,myB))
	{
	 ...
	 distance=sqrt((hmid.x-i)*(hmid.x-i)+(hmid.y-j)*(hmid.y-j)+(hmid.z-k)*(hmid.z-k));
		...
		if ((distance>prec_tmp)||(distance2>prec_tmp))
			...
			{
			//-- La boite est suffisamment pr�cise, on peut quitter la fonction
...
//---- Initialisation des param�tres caract�ristiques du bloc myB consid�r�  -----
...
//if (pourcentage<1)
//printf("Le grignotage a fontionn�  en %d iteration sur la boite %d %d %d  et la reduction est de %f \n",nb_iter,(int)i,(int)j,(int)k,result);   
...
*********		valeur de retour : pourcentage de r�duction de la boite
*********				
********************************************************************************/
...
if (myB->xm>myB->xM)
	printf("Il y a un probl�me pour x!!!\n");
...
//---- on consid�re les valeurs 4 coins des faces d'abscisse x
...
//Pour ce pr�munir des erreurs de precision 
...
if (myB->ym>myB->yM)
	printf("Il y a un probl�me pour y!!!\n");
...
//---- on consid�re les valeurs 4 coins des faces d'abscisse y
...
if (myB->zm>myB->zM)
	printf("Il y a un probl�me pour z!!!\n");
...
int l,stop;
size_depart=myB->zM-myB->zm;
//---- on consid�re les valeurs 4 coins des faces d'abscisse z
...
*********
*********		Propose un intervale xmin et xmax correspondant � un couple (y,z)
*********		
********************************************************************************/
...
*********
*********		Propose un intervale ymin et ymax correspondant � un couple (x,z)
*********		
********************************************************************************/
...
*********
*********		Propose un intervale ymin et ymax correspondant � un couple (x,y)
*********		
********************************************************************************/
...
*********
*********		Evaluation du champ de d�formation aux 8 sommets du bloc
*********   
********************************************************************************/
...
*********
*********		Essaie de trouver l'inverse sur une boite avec la m�thode
*********		du point fixe
*********    Retourne 1 si une solution avec une pr�cision prec est trouve, 0 sinon
********************************************************************************/
...
//free_dmatrix(invJ, 3, 3);
 ...
 printf("Nombre d'iteration depass� dans inv_bspline_find_solution_with_fixed_point_algo pour %f %f %f \n",i,j,k);
 free_dmatrix(J, 3, 3);
 free_dmatrix(invJ, 3, 3);
...
void imx_ComputeMoment3D_p(grphic3d * imdeb, grphic3d * imres, int p, int q, int r, float rayon)
{
 ...
 imx_dilat_3d_p(imdeb,tmp,4,2);//4 pour voisinage sph�rique sauf que je ne sais pas le rayon utilis�, 2 = nombre d'it�ration ca sert � quoi???
 res = alloc_dmatrix_3d(wdth, hght, dpth);

 //pr�paration du masque de convolution
...
//travailler sur une copi de imres puis utiliser la fonction imx_copi_3d_p dans imres, ceci �vite le probl�me quand src=dst dans l'affichage

...
/*---------------------------------------------------------------------------*/
/*----- retourne la valeur max autoris�e du pas -----------------------------*/
/*---------------------------------------------------------------------------*/
...
{ aux_i_old = *aux_i; aux_s_old = *aux_s;
  ...
  { pen = TOP_eval_poly(bsupx_s,bsupy_s,bsupz_s,Slpqr->alpha,1.0,0.0);
    if (pen<0)  																// pbs num�riques possibles (bsup_s<seuil et pen>0, d'o� pb si pas d'accolades)
  ...
  //----- pour �tre certain de se premunir contre d'improbables problemes numeriques...----
...
{
#ifndef TOPO_VERBOSE
printf("ATTENTION !!!!!!!!!!!! Probable pb num�rique !!!!!!!!!!!! *aux_s = %f    *aux_i= %f \n ",*aux_s,*aux_i);
#endif
}
 nb_iter++;
  ...
  { for (j=0; j<2; j++)
    { for (k=0; k<2; k++)
      { pente  = TOP_eval_poly(x[i],y[j],z[k],Slpqr->alpha,1,0);
        ...
        if ((ordorg<=Slpqr->Jm)||(ordorg>=Slpqr->JM))    		// possible suite � pb num�rique
...
%----- auteur  : C. Heinrich
%----- date    : juillet 2002
%----- objet   : proc�dure de minimisation
%-----           routine principale
%-----
%-----           retourne bornes inf et sup du minimum
%-----                    argument de la borne sup
%-----
%-----           recherche dans le pav� xinf,xsup,yinf,ysup,zinf,zsup
%-----
%-----           precis : pr�cision de la recherche
%-----           binf et bsup sont calcul�es sur des pav�s dont les dim < precis
%-----------------------------------------------------------
...
//    printf("%4d %4d %4d \n",niter,myQ.nelem,myL.nelem);
    ...
    myB = depile_end(&myQ);                     //myB = myQ(:,length(myQ(1,:)));    myQ = myQ(:,1:length(myQ(1,:))-1);         % on traite les petits pav�s d'abord pour �viter d'avoir une longue liste chain�e (casser des gros pav�s ne donne pas d'indication pr�cise sur l'encadrement du minimum)
  ...
  //binf = min(myL(7,:));                   //% et pas max(myL(7,:)) car ce n'est pas forc�ment le m�me minimum qui est encadr� dans chaque cas
...
%----- auteur  : C. Heinrich
%----- date    : juillet 2002
%----- objet   : contraction d'un pav� par encradrement du gradient
...
{ double mygrad[21][4]; double big_mat[21][10];
  ...
  *status = 0;
  ...
  for (i=1; i<=3; i++)                           // on traite successivement chacune des trois directions
  {
    ...
    modxi = 0; modxs = 0;                        // bool�ens indiquant la modification des bornes

    poly_marg_poly_fast(i,mygrad,big_mat);		 // ATTENTION : modification par rapport � la version matlab
    ...
    { if (xi-precis/3 > myinf[i])
      { myinf[i] = xi - precis / 3;
        myinf[i] = MIN2(mysup[i], myinf[i]);           // min([mysup(i) , myinf(i)]);   //% au cas o� on pourrait avoir myinf(i)>mysup(i) avec la ligne pr�c�dente, ce qui est possible
      }
    }
    ...
    { if (xs+precis/3 < mysup[i])
      { mysup[i] = xs + precis / 3;
        mysup[i] = MAX2(mysup[i], myinf[i]);                             // max([mysup(i) , myinf(i)]);                  % au cas o� on pourrait avoir mysup(i)<myinf(i) avec la ligne pr�c�dente, ce qui est possible
      }
    }
...
%-----           Valeurs des coefficients en un point donn�
%-----------------------------------------------------------
...
%-----------------------------------------------------------
% Sortie de la proc�dure : matrice
...
% forme de vecteur de coordonn�es dans la base des polynomes
% consid�r�s.
%-----------------------------------------------------------
...
//
// ATTENTION : modification des param�tres d'appel par rapport � la version matlab
//
// a ci-dessus correspond dans la fonction appelante � my_grad(:,param)
//
...
{ int i, j;
  ...
  //r = zeros(size(a));                                 //% pr�sence de r car commodit� MuPad
...
%----- auteur  : C. Heinrich
%----- date    : juillet 2002
%----- objet   : �valuation de la fonction associ�e au
%-----           polynome
%-----------------------------------------------------------
...
%----- objet   : �valuation de l'integrale du jacobien
%-----          
%-----------------------------------------------------------*/
...
%-----
%-----           Voir les commentaires en fin de proc�dure
%-----------------------------------------------------------
...
%----- auteur  : C. Heinrich
%----- date    : juillet 2002
%----- objet   : retourne une liste de pav�s resultant de
%-----           casser myB en xcass, ycass, zcass
%-----           bornes non initialis�es
%-----------------------------------------------------------


function [myL] = pave_casser(precis,myB,xcass,ycass,zcass);*/

// ATTENTION : xcass, ycass, zcass de dimension 2 n�cessairement
...
%-----           chaque coefficient
%-----
%-----           dir_des : vecteur colonne de dimension 3, norm�

...
public:
  ...
  virtual OffsetType IncrementInput( InputType & input ) const
    ...
    OffsetType out;
    if(input < m_HilbertPath.size() - 1){
            out = this->m_HilbertPath[input+1]-this->m_HilbertPath[input];
            input++;
    }else{
        out = this->m_HilbertPath[input]-this->m_HilbertPath[input];
        input++;
    }
    return out;
    }

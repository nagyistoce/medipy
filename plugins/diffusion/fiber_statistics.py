    ...
    for c1,c2 in zip(C_hits_1.keys(),C_hits_2.keys()):
        ...
        for cnt1 in range(shift_plane,l_pop-shift_plane):
            ...
            if points1.shape[0]>0:
                center1 = skel1[cnt1]
                points1 = cluster_voxel_2(points1,center1,weights1,dt1)
            if points2.shape[0]>0:
                center2 = skel2[cnt1]
                points2 = cluster_voxel_2(points2,center2,weights2,dt2)
            ...
            if N==nbClust:
                ...
                for point in points1:
                    t1.append(dt1[tuple(dt1.physical_to_index(point, True))])
                t1 = np.asarray(t1).T
                t2 = []
                for point in points2:
                    t2.append(dt2[tuple(dt2.physical_to_index(point, True))])
                t2 = np.asarray(t2).T
    ...
    for c1,c2 in zip(C_hits_1.keys(),C_hits_2.keys()):
        ...
        if kk:
            for cnt in range(N):
                ...
                pntref = dt1.physical_to_index(ref1[cnt], True)
                pop1_tmp = []
                pop2_tmp = []
...
def bundle_cross_sections_voxel(hits,model,cutoff,pv_thr) :
    ...
    for cnt,key in enumerate(hits.keys()) :
        ...
        for hit,center in zip(c_hits,skel) :
            ...
            for point in hit:
                index = model.physical_to_index(point, True)
                l = list(index==c)
...
def cluster_voxel(points, model):
    indices = [model.physical_to_index(p, True) for p in points]
    unique_indices = set([tuple(x) for x in indices])
    return np.asarray([np.asarray(x) for x in unique_indices])

def cluster_voxel_2(data,center,weights,model):
    ...
    for key in keys:
        ...
        tmp = np.asarray([model.index_to_physical(p) for p in data[idx]])
    ...
    indices = [model.physical_to_index(p, True) for p in points]
    unique_indices = set([tuple(x) for x in indices])
    unique_points = np.asarray([np.asarray(model.index_to_physical(x)) for x in unique_indices])

    d = np.sqrt( np.sum( (unique_points-center)**2,1 ) )
    argmax = np.argsort(d)
    unique_points = unique_points[argmax]

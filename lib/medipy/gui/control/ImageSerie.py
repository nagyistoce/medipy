...
class ImageSerie(wx.Panel, medipy.base.Observable):
    """ Control allowing the user to choose an item from a list of objects of 
        type "list of medipy.base.Image". If output is True, then a "(new)" 
        checkbox will be displayed ; when checked, the value of the control will
        be set to None, and it will be the caller's responsibility to perform the 
    ...
    def update_gui(self):
        ...
        if len(self.choices) == 0 :
            ...
            for i, choice in enumerate(self.choices) :
                ...
                is_image_list = (
                    isinstance(choice, (list, medipy.base.ObservableList)) and
                    all([isinstance(x, medipy.base.Image) for x in choice]))
                button.Enable(is_image_list and

...
class TestSchemes(unittest.TestCase):
    ...
    def test_dicomdir(self):
        ...
        url = "dicomdir:{0}".format(
            os.path.join(location, "BRAINIX", "DICOMDIR"))
        fragment = "{0}={1}".format("series_instance_uid", series_instance_uid)
        image1 = medipy.io.load("{0}#{1}".format(url, fragment))
        self.assertTrue(isinstance(image1, medipy.base.Image))
        
        tags = ["0x0020000e", "0020000e",
                "(0020,000e)", "(0x0020,0x000e)"]
        for tag in tags :
            fragment = "{0}={1}".format(tag, series_instance_uid)
            image2 = medipy.io.load("{0}#{1}".format(url, fragment))

...
def datafile_command(target, source, env):
    ...
    data_file.write(env["MODULE_NAME"] + os.linesep)
...
def datafile_emitter(target, source, env):
    target = [env["MODULE_NAME"]+".data"]
    return target, source    

def module_builder(env, module_name, classes, hint_file = ""):
    
    if hint_file != "" :
        hint_file = env.FindFile(hint_file, [""]).path
    ...
    for klass in classes :
        ...
        for suffix in source_suffixes :
            node = env.FindFile(klass+suffix, [""])
            if node is not None :
    ...
    module_init = env.VTKModuleInit(module_name+"_init.cpp", data_file)
    
    # 5. Replace the function called initlib<module_name> by init<module_name>
    def change_module_name(target, source, env):
        print target[0].path, source[0].path
        if sys.platform != "win32" :
            ...
            file.close()
    env.Command("dummy", module_init, change_module_name)
    
    # 6. Python Module
    sources = class_wrappers + module_init 
    for klass in files :
        if "source" in files[klass] :
            sources.append(os.path.basename(files[klass]["source"]))
    
    env.PythonModule(module_name, sources, CPPPATH=env["VTK"]["CPPPATH"],
...
def generate(env):
    ...
    module_init_command = "%s $SOURCE $TARGET"%env["VTK"]["vtk_wrap_python_init"]
